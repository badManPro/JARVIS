...
- 路径页与今日页现已补上 staged feedback：粗版刷新、今日计划生成/重生成都会展示分阶段反馈面板，并在完成后给出结构化结果卡
- 今日页步骤现已具备即时流转反馈、下一步自动上浮高亮，以及延期进入“明天候选区”的目标区域脉冲提示
- `app-shell` 现已接入桌面陪伴层最小容器，会基于 onboarding、priorityAction、riskSignals、todayPlan 和 scheduling 在“提醒 / 庆祝 / 状态反馈”之间切换，并明确保持“陪伴层，不是主入口”的边界
- 桌面陪伴层现已具备基础动作与表情状态：提醒会切到“前倾提醒 + 警觉聚焦”、庆祝会切到“轻跳庆祝 + 开心微笑”、状态反馈会切到“悬停同步 + 平静确认”，并通过显式状态标签与 `data-*` 标记为后续联动预留扩展点
- renderer store 现已提供短时 `companionCue` 通道；今日页的计划生成/步骤流转反馈与日历页的重排反馈会把 cue 推给桌面角色，让角色优先同步“刚刚发生了什么”，超时后再回落到原有 dashboard 总结逻辑
- 共享 `companion` 协议现已落地：短时 cue、跳转 action、人格语气和动作/表情映射都统一收敛到协议层，今日页与日历页可通过 `personaHint` 指定本次反馈更偏直接提醒、鼓励推进还是稳态解释，桌面角色面板也会显式展示当前人格基调与语气，为后续继续扩展动作和人格表现保留稳定接口
