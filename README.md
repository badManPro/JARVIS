...
- 对话页已支持把自然语言建议映射为结构化 action preview，并展示目标/计划变更的预览卡片
...
- 对话页中的建议动作预览卡片，会基于当前状态把字符串建议回填为结构化 action preview
...
- 对话相关数据仍主要保留在 `app_snapshots`，但主进程会在加载时把 `conversation.suggestions` 回填为结构化 `actionPreviews`
- 当前计划草案仍由本地规则模板生成，尚未接入真实 AI Provider 生成 / 重排
- 当前尚未提供 action preview 的确认/拒绝落库、版本回滚、目标排序等更高阶动作

## 下一步建议
1. 支持对画像、目标、计划三类变更做可确认预览
2. 支持用户确认后将结构化动作真正落库
3. 增加统一 AI service，按 capability route 到不同 Provider
4. 继续减少 `app_snapshots` 对业务实体的兜底职责，补充更稳妥的迁移机制

## 当前推荐下一任务
- `Phase 2 / Task 2`：支持对画像、目标、计划三类变更做可确认预览
