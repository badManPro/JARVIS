...
### 当前最该做的事情
- **P1 / Task 1：为学习计划补齐“手动编辑 + 重新生成入口 + 版本对比基础结构”。**

这一步是后续对话调整、AI 生成、复盘重排的共同前置条件。
...
| Phase 1 | 计划编辑与计划生命周期 | pending | 15% | 让计划真正可编辑、可重生、可比较 |
| Phase 2 | 对话驱动结构化动作 | pending | 5% | 让对话能把建议落成可确认动作 |
...
## 8. 当前建议的最近三项任务
1. **Phase 1 / Task 1**：学习计划页支持手动编辑阶段与任务，并落库到 `learning_plan_drafts`、`plan_stages`、`plan_tasks`
2. **Phase 1 / Task 2**：增加计划“重新生成”按钮、确认弹层和版本快照
3. **Phase 1 / Task 3**：增加版本对比视图，只展示标题、阶段、任务三类核心差异
