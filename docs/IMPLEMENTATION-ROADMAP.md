...
### 当前总进度
- **整体进度估算：97%**
- **当前阶段：Phase 5 进行中，下一任务为 `Phase 5 / Task 4`（为删除 / 重排 / 重生成等高风险动作补事务保护）**
...
这一步应该把目标删除、计划重生成、批量应用 action preview 等高风险写操作包进更清晰的事务边界，避免主进程在中途失败时留下半完成状态。
...
- **当前进展：**
  ...
  - 项目当前唯一下一任务：**Phase 5 / Task 4 / 为删除 / 重排 / 重生成等高风险动作补事务保护**
- **本阶段完成后指定的下一个任务：**
  - **Next Task: Phase 5 / Task 4 / 为删除 / 重排 / 重生成等高风险动作补事务保护**
