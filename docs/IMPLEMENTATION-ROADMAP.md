...
### 当前总进度
- **整体进度估算：80%**
- **当前阶段：Phase 4 已进入 `Task 2`，下一任务为 `Phase 4 / Task 2`（日 / 周 / 阶段复盘输入）**
...
这一步会在已有任务执行数据之上补齐用户可编辑的复盘输入，让后续 `reflection_summary` 和计划调整真正拥有可用上下文。
...
| Phase 4 | 执行与复盘闭环 | in_progress | 25% | 让任务执行、复盘、计划调整形成闭环 |
...
- **当前进展：**
  ...
  - 首页与复盘页已改为消费真实任务执行派生摘要，能立即看到今日聚焦、风险提醒、完成统计与最近执行记录
  - 项目当前唯一下一任务：**Phase 4 / Task 2 / 日 / 周 / 阶段复盘输入**
- **本阶段完成后指定的下一个任务：**
...
## 8. 当前建议的最近三项任务
1. **Phase 4 / Task 2**：日 / 周 / 阶段复盘输入
2. **Phase 4 / Task 3**：复盘结果影响画像与计划的建议生成
