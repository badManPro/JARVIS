...
- 核心 AI capability 已接入主要业务入口，请求日志与最小可观测性也已补齐；最新一轮学习驾驶舱信息架构收口也已完成，当前只剩发布候选阶段的手工走查、打包验证和演示准备。

## 2. 当前判断

### 当前总进度
- **整体进度估算：100%**
- **当前阶段：Learning Cockpit Follow-Up 已完成，下一任务为 `Release Candidate`（手工走查、打包与演示准备）**

> 说明：这个百分比是基于“产品可用闭环”而不是“代码文件数量”估算的。基础功能阶段已经完成，最近一轮工作主要用于把主路径收口成更长期可维护的四页驾驶舱结构。
...
- 计划页已支持对单个任务执行开始 / 完成 / 跳过 / 延后，相关状态原因和最近流转时间会落到 `plan_tasks`。
- 首页与复盘页已开始消费真实任务执行数据，可立即看到今日聚焦、完成统计、偏差摘要与最近执行记录。
...
- 一级导航已收敛为 `today / path / profile / settings`，全局教练抽屉成为首次建档、“我有变化”和建议确认的统一入口。
- `dashboard-content.tsx` 已拆分为按职责分离的 dashboard 子组件，主路径逻辑不再继续堆叠在单文件里。
- 教练抽屉已直接展示 `conversation.actionPreviews`，支持 accept / reject / apply 闭环，并在应用后同步刷新当前状态、优先跳转 `today / path`。
- 设置页已完成分层：默认层只保留主题、启动页和 Codex 连接，高级设置承载 route、runtime、observability 与 Provider 编辑。
- 复盘已从独立一级页面退出；今日页在任务完成 / 延后 / 跳过后直接弹出轻量复盘，路径页提供阶段复盘入口。
- 学习路径首屏已收敛为当前主目标、当前阶段和最近任务，完整路径依据与历史快照改为渐进披露。

### 当前最该做的事情
- **Release Candidate：手工走查、打包验证与演示准备。**

这一步应该把已经完成实现和自动化验证的应用，推进到最终手工走查、演示路径确认、打包产物验证和发布候选物收口。
...
- **当前进展：**
  ...
  - 本阶段已完成，项目当前已推进到 **Phase 4 / Task 2 / 日 / 周 / 阶段复盘输入**
- **本阶段完成后指定的下一个任务：**
  - **Next Task: Phase 4 / Task 2 / 日 / 周 / 阶段复盘输入**
...
- **风险：**
  - 如果没有执行数据，计划永远只是静态内容
- **当前进展：**
  - Task 1 已完成：计划页支持对单个任务执行开始 / 完成 / 跳过 / 延后，`plan_tasks` 会记录 `status_note` 与 `status_updated_at`
  ...
  - 项目当前唯一下一任务：**Phase 6 / Task 1 / 完成端到端验收、打包和发布前检查**
- **本阶段完成后指定的下一个任务：**
...
- **当前进展：**
  ...
  - Task 3 已完成：主进程新增一致性归一化层，会在 load/save 路径修复 stale plan snapshot 引用和指向缺失 provider 的 route，并把结构化修复结果回写到 SQLite
  - Task 4 已完成：`AppStorageService` 现通过 `AppStateRepository.transaction()` 把删除目标、保存/重排计划草案、计划重生成、批量应用 action preview、建议回流与执行/复盘写入收口为单次原子提交；对应回滚测试覆盖了目标删除、草案重排与计划重生成的失败场景
  - 项目当前唯一下一任务：**Phase 6 / Task 1 / 完成端到端验收、打包和发布前检查**
- **本阶段完成后指定的下一个任务：**
  - **Next Task: Phase 6 / Task 1 / 完成端到端验收、打包和发布前检查**
