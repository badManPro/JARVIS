...
test('app store exposes a transient companion cue channel for cross-page desktop feedback', () => {
  assert.match(storeSource, /@shared\/companion/);
  assert.match(storeSource, /companionCue: null/);
  assert.match(storeSource, /import type \{ CompanionCue, CompanionCueInput \} from '@shared\/companion'/);
  ...
  assert.match(storeSource, /createCompanionCue\(cue: CompanionCueInput\)/);
